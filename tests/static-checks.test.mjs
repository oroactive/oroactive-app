...
test("laboratorio gemmologico pubblica solo schede complete e approvate", async () => {
  ...
  for (const material of ACADEMY_GEM_MATERIALS) {
    ...
    assert.equal(material.published, true, `${material.slug}: scheda completa non pubblicata`);
    assert.equal(material.review_status, "approved", `${material.slug}: revisione scientifica assente`);
    assert.equal(material.media_status, "approved", `${material.slug}: media non approvati`);
    assert.equal(material.founder_review_status, "approved", `${material.slug}: revisione editoriale assente`);
...
test("PWA non cachea API e dati sensibili", async () => {
  ...
  assert.doesNotMatch(server, /express\.static\(__dirname/);
  assert.match(server, /publicRootStaticFiles = new Set/);
  assert.match(server, /app\.use\("\/assets\/coins", express\.static/);
  assert.match(server, /isBlockedPath \|\| path\.posix\.extname\(requestPath\)/);
  assert.match(app, /window\.__OROACTIVE_DIRTY_STATE__ = false/);
  assert.match(app, /window\.__OROACTIVE_VERSION__ = null/);
  assert.match(app, /const OROACTIVE_UPDATE_INTERVAL_MS = 900000/);
...
test("splash screen iniziale premium animata e senza ghost screen", async () => {
  ...
  assert.match(app, /const OROACTIVE_SPLASH_MIN_MS = 300/);
  assert.match(app, /const OROACTIVE_SPLASH_BRIEF_MS = 120/);
...
test("Elenco Monete è una sottosezione Formazione con riconoscimento foto backend", async () => {
  ...
  assert.match(app, /id: "sterlina-oro-2000-fdc"[\s\S]*name: "Sterlina oro 2000 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2000"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2000/i);
  ...
  assert.match(app, /id: "sterlina-oro-2019-fdc"[\s\S]*name: "Sterlina oro 2019 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2019"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2019/i);
  assert.match(app, /id: "sterlina-oro-2020-fdc"[\s\S]*name: "Sterlina oro 2020 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2020"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2020/i);
  assert.match(app, /id: "sterlina-oro-2021-fdc"[\s\S]*name: "Sterlina oro 2021 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2021"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2021/i);
  assert.match(app, /id: "sterlina-oro-2022-platinum-jubilee-fdc"[\s\S]*name: "Sterlina oro 2022 - Platinum Jubilee \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2022"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /platinum jubilee sovereign/i);
  assert.match(app, /id: "sterlina-oro-2023-re-carlo-iii-coronation-sovereign-fdc"[\s\S]*name: "Sterlina oro 2023 - Re Carlo III - Coronation Sovereign \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /coronation sovereign/i);
  assert.match(app, /id: "sterlina-oro-2024-re-carlo-iii-fdc"[\s\S]*name: "Sterlina oro 2024 - Re Carlo III \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2024"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2024/i);
  assert.match(app, /id: "sterlina-oro-2025-re-carlo-iii-fdc"[\s\S]*name: "Sterlina oro 2025 - Re Carlo III \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2025"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2025/i);
  assert.match(app, /id: "sterlina-oro-2026-re-carlo-iii-fdc"[\s\S]*name: "Sterlina oro 2026 - Re Carlo III \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2026"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /sovereign 2026/i);
  ...
  assert.match(app, /Girls of Great Britain and Ireland/);
  assert.match(app, /id: "sterlina-oro-elisabetta-ii-fiocco"[\s\S]*name: "Sterlina oro Elisabetta II \(Fiocco\)"[\s\S]*mintYears: "1957-1968"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /Mary Gillick/);
  assert.match(app, /fiocchetto/);
  assert.match(app, /id: "sterlina-oro-vittoria-stemmata"[\s\S]*name: "Sterlina oro Vittoria \(Stemmata\)"[\s\S]*mintYears: "1838-1871"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /Vittoria Testa Giovane con Stemma/);
  assert.match(app, /BRITANNIARUM REGINA FID DEF/);
  assert.match(app, /id: "sterlina-oro-vittoria-giovane"[\s\S]*name: "Sterlina oro Vittoria \(Giovane\)"[\s\S]*mintYears: "1871-1887"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /Victoria Young Head/);
  assert.match(app, /id: "sterlina-oro-vittoria-coronata"[\s\S]*name: "Sterlina oro Vittoria \(Coronata\)"[\s\S]*mintYears: "1887-1893"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /Victoria Jubilee Head/);
  assert.match(app, /Testa Coronata/);
  assert.match(app, /id: "sterlina-oro-vittoria-velata"[\s\S]*name: "Sterlina oro Vittoria \(Velata\)"[\s\S]*mintYears: "1893-1901"[\s\S]*grossWeight: 7\.98,[\s\S]*diameter: 22\.05/);
  assert.match(app, /Victoria Old Head/);
  assert.match(app, /Testa Velata/);
  assert.match(app, /id: "doppia-sterlina-oro-2022-fdc"[\s\S]*name: "Doppia Sterlina oro 2022 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2022"[\s\S]*grossWeight: 15\.98,[\s\S]*diameter: 28\.4/);
  assert.match(app, /platinum jubilee double sovereign/i);
  assert.match(app, /id: "doppia-sterlina-oro-2026-fdc"[\s\S]*name: "Doppia Sterlina oro 2026 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2026"[\s\S]*grossWeight: 15\.98,[\s\S]*diameter: 28\.4/);
  assert.match(app, /Double Sovereign 2026/);
  assert.match(app, /micro-incisioni/);
  ...
  assert.match(app, /id: "quarto-di-sterlina-oro-2022-platinum-jubilee-fdc"[\s\S]*name: "Quarto di Sterlina oro 2022 Platinum Jubilee \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2022"[\s\S]*grossWeight: 1\.83,[\s\S]*diameter: 13\.5/);
  assert.match(app, /quarter sovereign 2022 platinum jubilee/i);
  ...
  assert.match(app, /EDWARDVS VII/);
  assert.match(app, /id: "mezza-sterlina-oro-giorgio-v-anni-misti"[\s\S]*name: "Mezza Sterlina oro Giorgio V \(anni misti\)"[\s\S]*nominal: "1\/2 Sovereign"[\s\S]*grossWeight: 3\.99,[\s\S]*fineGold: 3\.66,[\s\S]*diameter: 19\.2/);
  assert.match(app, /GEORGIVS V/);
  assert.match(app, /id: "mezza-sterlina-oro-vittoria-velata-anni-misti"[\s\S]*name: "Mezza Sterlina oro Vittoria \(Velata, anni misti\)"[\s\S]*nominal: "1\/2 Sovereign"[\s\S]*grossWeight: 3\.99,[\s\S]*fineGold: 3\.66,[\s\S]*diameter: 19\.2/);
  assert.match(app, /Victoria Old Head/);
  ...
  assert.match(app, /id: "10-marchi-amburgo"[\s\S]*country: "Germania"[\s\S]*mintYears: "1873-1913"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /FREIE UND HANSESTADT HAMBURG/);
  assert.match(app, /DEUTSCHES REICH/);
  assert.match(server, /10-marchi-amburgo/);
  assert.match(server, /10 Marchi Citta Libera Anseatica di Amburgo/);
  assert.match(app, /id: "10-marchi-guglielmo-i-prussia"[\s\S]*country: "Germania"[\s\S]*mintYears: "1873-1888"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /WILHELM DEUTSCHER KAISER KONIG V\. PREUSSEN/);
  assert.match(server, /10-marchi-guglielmo-i-prussia/);
  assert.match(server, /10 Marchi d'oro Imperatore Guglielmo I di Prussia/);
  assert.match(app, /id: "10-marchi-guglielmo-ii-prussia"[\s\S]*country: "Germania"[\s\S]*mintYears: "1889-1913"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /WILHELM II DEUTSCHER KAISER KONIG V\. PREUSSEN/);
  assert.match(server, /10-marchi-guglielmo-ii-prussia/);
  assert.match(app, /id: "10-marchi-ludovico-ii-baviera"[\s\S]*country: "Germania"[\s\S]*mintYears: "1874-1886"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /LUDWIG II KOENIG V\. BAYERN/);
  assert.match(server, /10-marchi-ludovico-ii-baviera/);
  assert.match(server, /10 Marchi Re Ludovico II di Baviera/);
  assert.match(app, /id: "10-marchi-federico-i-baden"[\s\S]*country: "Germania"[\s\S]*mintYears: "1872-1901"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /FRIEDRICH GROSSHERZOG VON BADEN/);
  assert.match(server, /10-marchi-federico-i-baden/);
  assert.match(app, /id: "10-marchi-carlo-wurttemberg"[\s\S]*country: "Germania"[\s\S]*mintYears: "1864-1891"[\s\S]*grossWeight: 3\.58,[\s\S]*fineGold: 3\.222,[\s\S]*diameter: 19\.5,[\s\S]*thickness: 0\.9/);
  assert.match(app, /KARL KONIG VON WUERTTEMBERG/);
  assert.match(server, /10-marchi-carlo-wurttemberg/);
  ...
  assert.match(app, /1\/4 OZ FINE GOLD/);
  assert.match(app, /1\/10 OZ FINE GOLD/);
  ...
  assert.match(app, /id: "britannia-1-oz"[\s\S]*name: "100 Sterline Britannia d'oro"[\s\S]*mintYears: "Anni misti \/ 1987-oggi"[\s\S]*purityLabel: "916,7‰ fino al 2012 \/ 999,9‰ dal 2013"[\s\S]*grossWeight: 31\.10,[\s\S]*diameter: 32\.61/);
  assert.match(app, /Capital Gains Tax|capital gain tax/i);
  assert.match(app, /Rank-Broadley/);
  assert.match(app, /id: "britannia-100-sterline-fdc-carlo-iii"[\s\S]*name: "100 Sterline Britannia d'oro \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 31\.10,[\s\S]*diameter: 32\.61/);
  assert.match(app, /CHARLES III D\.G\. REX F\.D\./);
  assert.match(app, /id: "incoronazione-carlo-iii-100-sterline-oro"[\s\S]*name: "100 Sterline Incoronazione Re Carlo III"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 31\.10,[\s\S]*diameter: 32\.61/);
  assert.match(app, /Martin Jennings/);
  assert.match(app, /tiratura limitata di 7\.500/);
  assert.match(app, /id: "britannia-liberty-2024-100-sterline-oro-fdc"[\s\S]*name: "100 Sterline oro Britannia & Liberty 2024 \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2024"[\s\S]*grossWeight: 31\.10,[\s\S]*diameter: 32\.61/);
  assert.match(app, /tiratura limitata a 11\.000/);
  assert.match(app, /id: "britannia-50-sterline-1-2-oz"[\s\S]*name: "50 Sterline Britannia d'oro"[\s\S]*mintYears: "Anni misti \/ 1987-oggi"[\s\S]*grossWeight: 15\.59,[\s\S]*diameter: 27/);
  assert.match(app, /1\/2 oncia/);
  assert.match(app, /id: "britannia-50-sterline-fdc-carlo-iii"[\s\S]*name: "50 Sterline Britannia d'oro \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 15\.59,[\s\S]*diameter: 27/);
  assert.match(app, /id: "britannia-25-sterline-1-4-oz"[\s\S]*name: "25 Sterline Britannia d'oro"[\s\S]*mintYears: "Anni misti \/ 1987-oggi"[\s\S]*grossWeight: 7\.80,[\s\S]*diameter: 22/);
  assert.match(app, /id: "britannia-25-sterline-fdc-carlo-iii"[\s\S]*name: "25 Sterline Britannia d'oro \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 7\.80,[\s\S]*diameter: 22/);
  assert.match(app, /1\/4 di oncia/);
  assert.match(app, /id: "britannia-10-sterline-1-10-oz"[\s\S]*name: "10 Sterline Britannia d'oro"[\s\S]*mintYears: "Anni misti \/ 1987-oggi"[\s\S]*grossWeight: 3\.11,[\s\S]*diameter: 16\.5/);
  assert.match(app, /id: "britannia-10-sterline-fdc-carlo-iii"[\s\S]*name: "10 Sterline Britannia d'oro \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 3\.11,[\s\S]*diameter: 16\.5/);
  assert.match(app, /1\/10 di oncia/);
  assert.match(app, /id: "britannia-100-sterline-2023-elisabetta-ii-fdc"[\s\S]*name: "100 Sterline Britannia d'oro 2023 Regina Elisabetta II \(FIOR DI CONIO\)"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 31\.10,[\s\S]*diameter: 32\.61/);
  assert.match(app, /Decus et Tutamen/);
  assert.doesNotMatch(app, /britannia-10-sterline-oro/);
  ...
  assert.match(app, /tudor-beasts-toro-clarence-2023/);
  assert.match(app, /Tudor Beasts Toro di Clarence 2023/);
  assert.match(app, /id: "tudor-beasts-toro-clarence-2023"[\s\S]*country: "Regno Unito"[\s\S]*mintYears: "Fior di Conio \(FDC\) \/ 2023"[\s\S]*grossWeight: 31\.21,[\s\S]*diameter: 32\.69/);
  assert.match(app, /bull of clarence/);
  assert.match(app, /Martin Jennings/);
  ...
  assert.match(app, /groupedCoinsByCountry\(pagedCoins\)\.map\(coinCountryGroupMarkup\)/);
  ...
  assert.match(server, /sterlina-oro-2000-fdc/);
  assert.match(server, /Sterlina oro 2000 \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2000/);
  ...
  assert.match(server, /sterlina-oro-2019-fdc/);
  assert.match(server, /Sterlina oro 2019 \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2019/);
  assert.match(server, /sterlina-oro-2020-fdc/);
  assert.match(server, /Sterlina oro 2020 \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2020/);
  assert.match(server, /sterlina-oro-2021-fdc/);
  assert.match(server, /Sterlina oro 2021 \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2021/);
  assert.match(server, /sterlina-oro-2022-platinum-jubilee-fdc/);
  assert.match(server, /Sterlina oro 2022 - Platinum Jubilee \(FIOR DI CONIO\)/);
  assert.match(server, /platinum jubilee sovereign/i);
  assert.match(server, /sterlina-oro-2023-re-carlo-iii-coronation-sovereign-fdc/);
  assert.match(server, /Sterlina oro 2023 - Re Carlo III - Coronation Sovereign \(FIOR DI CONIO\)/);
  assert.match(server, /coronation sovereign/i);
  assert.match(server, /sterlina-oro-2024-re-carlo-iii-fdc/);
  assert.match(server, /Sterlina oro 2024 - Re Carlo III \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2024/i);
  assert.match(server, /sterlina-oro-2025-re-carlo-iii-fdc/);
  assert.match(server, /Sterlina oro 2025 - Re Carlo III \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2025/i);
  assert.match(server, /sterlina-oro-2026-re-carlo-iii-fdc/);
  assert.match(server, /Sterlina oro 2026 - Re Carlo III \(FIOR DI CONIO\)/);
  assert.match(server, /sovereign 2026/i);
  assert.match(server, /sterlina-oro-elisabetta-ii-coroncina/);
  assert.match(server, /Sterlina oro Elisabetta II \(Coroncina\)/);
  assert.match(server, /arnold machin/i);
  assert.match(server, /sterlina-oro-elisabetta-ii-fiocco/);
  assert.match(server, /Sterlina oro Elisabetta II \(Fiocco\)/);
  assert.match(server, /mary gillick/i);
  assert.match(server, /sterlina-oro-vittoria-stemmata/);
  assert.match(server, /Sterlina oro Vittoria \(Stemmata\)/);
  assert.match(server, /victoria shield sovereign/i);
  assert.match(server, /sterlina-oro-vittoria-giovane/);
  assert.match(server, /Sterlina oro Vittoria \(Giovane\)/);
  assert.match(server, /victoria young head/i);
  assert.match(server, /sterlina-oro-vittoria-coronata/);
  assert.match(server, /Sterlina oro Vittoria \(Coronata\)/);
  assert.match(server, /victoria jubilee head/i);
  assert.match(server, /sterlina-oro-vittoria-velata/);
  assert.match(server, /Sterlina oro Vittoria \(Velata\)/);
  assert.match(server, /victoria old head/i);
  assert.match(server, /doppia-sterlina-oro-2022-fdc/);
  assert.match(server, /Doppia Sterlina oro 2022 \(FIOR DI CONIO\)/);
  assert.match(server, /platinum jubilee double sovereign/i);
  assert.match(server, /doppia-sterlina-oro-2026-fdc/);
  assert.match(server, /Doppia Sterlina oro 2026 \(FIOR DI CONIO\)/);
  assert.match(server, /double sovereign 2026/i);
  ...
  assert.match(server, /quarto-di-sterlina-oro-2022-platinum-jubilee-fdc/);
  assert.match(server, /Quarto di Sterlina oro 2022 Platinum Jubilee \(FIOR DI CONIO\)/);
  assert.match(server, /quarter sovereign 2022 platinum jubilee/i);
  ...
  assert.match(server, /mezza-sterlina-oro-edoardo-vii-anni-misti/);
  assert.match(server, /Mezza Sterlina oro Edoardo VII \(anni misti\)/);
  assert.match(server, /half sovereign edward vii/i);
  assert.match(server, /mezza-sterlina-oro-giorgio-v-anni-misti/);
  assert.match(server, /Mezza Sterlina oro Giorgio V \(anni misti\)/);
  assert.match(server, /half sovereign george v/i);
  assert.match(server, /mezza-sterlina-oro-vittoria-velata-anni-misti/);
  assert.match(server, /Mezza Sterlina oro Vittoria \(Velata, anni misti\)/);
  assert.match(server, /half sovereign victoria old head/i);
  assert.match(server, /somalia-elephant-2023-1-oz/);
  assert.match(server, /arca-noe-armenia-2025-1-oz/);
  assert.match(server, /100 Sterline Britannia d'oro/);
  assert.match(server, /Rank-Broadley/);
  assert.match(server, /916,7 per mille fino al 2012; 999,9 per mille dal 2013/);
  assert.match(server, /britannia-100-sterline-fdc-carlo-iii/);
  assert.match(server, /100 Sterline Britannia d'oro \(FIOR DI CONIO\)/);
  assert.match(server, /charles iii/i);
  assert.match(server, /incoronazione-carlo-iii-100-sterline-oro/);
  assert.match(server, /100 Sterline Incoronazione Re Carlo III/);
  assert.match(server, /martin jennings/i);
  assert.match(server, /britannia-liberty-2024-100-sterline-oro-fdc/);
  assert.match(server, /100 Sterline oro Britannia & Liberty 2024 \(FIOR DI CONIO\)/);
  assert.match(server, /united states mint/i);
  assert.match(server, /britannia-50-sterline-1-2-oz/);
  assert.match(server, /50 Sterline Britannia d'oro/);
  assert.match(server, /1\/2 oz/);
  assert.match(server, /britannia-50-sterline-fdc-carlo-iii/);
  assert.match(server, /50 Sterline Britannia d'oro \(FIOR DI CONIO\)/);
  assert.match(server, /britannia-25-sterline-1-4-oz/);
  assert.match(server, /25 Sterline Britannia d'oro/);
  assert.match(server, /1\/4 oz/);
  assert.match(server, /britannia-25-sterline-fdc-carlo-iii/);
  assert.match(server, /25 Sterline Britannia d'oro \(FIOR DI CONIO\)/);
  assert.match(server, /britannia-10-sterline-1-10-oz/);
  assert.match(server, /10 Sterline Britannia d'oro/);
  assert.match(server, /1\/10 oz/);
  assert.match(server, /britannia-10-sterline-fdc-carlo-iii/);
  assert.match(server, /10 Sterline Britannia d'oro \(FIOR DI CONIO\)/);
  assert.match(server, /britannia-100-sterline-2023-elisabetta-ii-fdc/);
  assert.match(server, /100 Sterline Britannia d'oro 2023 Regina Elisabetta II \(FIOR DI CONIO\)/);
  ...
  assert.match(server, /america-aquila-25-dollari-oro/);
  assert.match(server, /America Aquila 25 Dollari d'oro/);
  assert.match(server, /1\/2 oz fine gold/i);
  assert.match(server, /america-aquila-10-dollari-oro/);
  assert.match(server, /America Aquila 10 Dollari d'oro/);
  assert.match(server, /1\/4 oz fine gold/i);
  ...
  assert.match(app, /function coinMiniFacesMarkup\(coin = \{\}\) \{[\s\S]*coinFaceMarkup\(coin, "front", \{ thumbnail: true \}\)[\s\S]*coinFaceMarkup\(coin, "back", \{ thumbnail: true \}\)/);
  ...
  await Promise.all([
    ...
    access(new URL("assets/coins/bilancia-oro/tudor-beasts-toro-clarence-2023-front.png", root)),
    access(new URL("assets/coins/bilancia-oro/tudor-beasts-toro-clarence-2023-back.png", root)),
...
test("tutte le monete del catalogo hanno foto fronte retro presenti", async () => {
  ...
  for (const coin of catalog) {
    ...
    for (const side of ["front", "back"]) {
      ...
      const thumbnailName = relativePath.split("/").pop().replace(/\.[^.]+$/, ".webp");
      await access(new URL(`assets/coins/thumbnails/${thumbnailName}`, root));
    }
  }
  ...
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-amburgo-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-amburgo-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-guglielmo-i-prussia-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-guglielmo-i-prussia-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-guglielmo-ii-prussia-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-guglielmo-ii-prussia-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-ludovico-ii-baviera-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-ludovico-ii-baviera-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-federico-i-baden-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-federico-i-baden-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-carlo-wurttemberg-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/10-marchi-carlo-wurttemberg-back.png"), true);
  ...
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2019-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2019-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2020-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2020-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2021-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2021-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2022-platinum-jubilee-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2022-platinum-jubilee-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2023-re-carlo-iii-coronation-sovereign-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2023-re-carlo-iii-coronation-sovereign-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2024-re-carlo-iii-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2024-re-carlo-iii-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2025-re-carlo-iii-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2025-re-carlo-iii-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2026-re-carlo-iii-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-2026-re-carlo-iii-fdc-back.png"), true);
  ...
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-elisabetta-ii-fiocco-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-stemmata-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-stemmata-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-giovane-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-giovane-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-coronata-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-coronata-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-velata-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/sterlina-oro-vittoria-velata-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/doppia-sterlina-oro-2022-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/doppia-sterlina-oro-2022-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/doppia-sterlina-oro-2026-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/doppia-sterlina-oro-2026-fdc-back.png"), true);
  ...
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/quarto-di-sterlina-oro-2022-platinum-jubilee-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/quarto-di-sterlina-oro-2022-platinum-jubilee-fdc-back.png"), true);
  ...
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/mezza-sterlina-oro-edoardo-vii-anni-misti-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/mezza-sterlina-oro-giorgio-v-anni-misti-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/mezza-sterlina-oro-giorgio-v-anni-misti-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/mezza-sterlina-oro-vittoria-velata-anni-misti-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/mezza-sterlina-oro-vittoria-velata-anni-misti-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-1-oz-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-1-oz-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-100-sterline-fdc-carlo-iii-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-100-sterline-fdc-carlo-iii-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/incoronazione-carlo-iii-100-sterline-oro-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/incoronazione-carlo-iii-100-sterline-oro-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-liberty-2024-100-sterline-oro-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-liberty-2024-100-sterline-oro-fdc-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-50-sterline-1-2-oz-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-50-sterline-1-2-oz-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-50-sterline-fdc-carlo-iii-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-50-sterline-fdc-carlo-iii-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-25-sterline-1-4-oz-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-25-sterline-1-4-oz-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-25-sterline-fdc-carlo-iii-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-25-sterline-fdc-carlo-iii-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-10-sterline-1-10-oz-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-10-sterline-1-10-oz-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-10-sterline-fdc-carlo-iii-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-10-sterline-fdc-carlo-iii-back.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-100-sterline-2023-elisabetta-ii-fdc-front.png"), true);
  assert.equal(referencedImages.has("assets/coins/bilancia-oro/britannia-100-sterline-2023-elisabetta-ii-fdc-back.png"), true);
});
...
test("mascotte Aurum interattiva usa gufo dorato, flag e AI esistente", async () => {
  ...
  assert.match(index, /Statistiche memorie Aurum/);
  assert.match(index, /id="aurumMyMemoriesList"/);
  assert.match(index, /id="aurumExportMemories"/);
  assert.match(index, /id="aurumDeleteAllMemories"/);
  assert.match(index, /id="aurumShareMemoryWithAi"/);
  ...
  assert.match(app, /function isAurumFiscalAccountingQuestion/);
  assert.match(app, /function isAurumProfessionalGoldQuestion/);
  assert.match(app, /function isAurumFoundryQuestion/);
  assert.match(app, /function isAurumAuthoritiesSuapOpeningQuestion/);
  assert.match(app, /function buildAurumNormativeAnswer/);
  assert.match(app, /function aurumSourcesMarkup/);
  assert.match(app, /Fonti e riferimenti/);
  assert.match(app, /const wantsCurrentWebVerification = normativeQuestion/);
  assert.match(app, /allowWeb: wantsCurrentWebVerification/);
  assert.match(app, /function isAurumFieldHelpQuestion/);
  assert.match(app, /normativa_operativa/);
  assert.match(app, /D\.Lgs\. 25 maggio 2017, n\. 92/);
  assert.match(app, /D\.Lgs\. 10 dicembre 2024, n\. 211/);
  ...
  assert.match(askAurumBlock, /const accountingQuestion = isAurumFiscalAccountingQuestion\(question\)/);
  assert.match(askAurumBlock, /const professionalGoldQuestion = isAurumProfessionalGoldQuestion\(question\)/);
  assert.match(askAurumBlock, /const foundryQuestion = isAurumFoundryQuestion\(question\)/);
  assert.match(askAurumBlock, /const authoritiesSuapQuestion = isAurumAuthoritiesSuapOpeningQuestion\(question\)/);
  assert.match(askAurumBlock, /const salesQuestion = isAurumSalesCommunicationQuestion\(question\)/);
  assert.match(askAurumBlock, /const normativeQuestion = authoritiesSuapQuestion \|\| \(!salesQuestion && isAurumNormativeQuestion\(question\)\) \|\| accountingQuestion \|\| professionalGoldQuestion \|\| foundryQuestion/);
  assert.match(askAurumBlock, /!normativeQuestion && !salesQuestion && !extractionQuestion && handleAurumTutorRequest\(question\)/);
  assert.match(askAurumBlock, /!accountingQuestion && !professionalGoldQuestion && !foundryQuestion && !isAurumNormativeAnswerAdequate\(data\.risposta\)/);
  ...
  assert.doesNotMatch(app, /function recordAurumInteractionMemory/);
  assert.doesNotMatch(app, /automatic:\s*true/);
  assert.match(app, /explicit_consent:\s*true/);
  ...
  assert.match(server, /function isComproOroAccountingQuestion/);
  assert.match(server, /function buildComproOroNormativeAnswer/);
  assert.match(server, /AURUM_SECTOR_KNOWLEDGE/);
  assert.match(server, /searchSectorKnowledge/);
  assert.match(server, /formatSectorKnowledgeContext/);
  assert.match(server, /Base settoriale Aurum verificata/);
  assert.match(server, /filterTrustedAssistantWebResults/);
  assert.match(server, /redactAssistantPersonalData/);
  assert.match(server, /containsAssistantPersonalData/);
  assert.match(server, /sanitizeAssistantContextObject/);
  assert.match(server, /sanitizeAssistantUntrustedContext/);
  assert.match(server, /Nessuna memoria personale autorizzata alla condivisione con il modello IA/);
  assert.match(server, /Invio esterno bloccato per tutela dei dati personali/);
  assert.match(server, /"camcom\.it"/);
  assert.match(server, /"poliziadistato\.it"/);
  assert.match(server, /"rsc\.org"/);
  assert.match(server, /"agenziaentrate\.gov\.it"/);
  assert.match(server, /"fondazioneoic\.eu"/);
  assert.match(server, /"inps\.it"/);
  assert.match(server, /safeAurumSectionContext/);
  assert.match(server, /safePriceExplanationText/);
  assert.match(server, /safeNormativeText/);
  assert.match(server, /sanitizeAssistantUntrustedContext\(chunk\.content, 5000\)/);
  assert.match(server, /ignora qualunque istruzione/);
  assert.match(server, /DELETE FROM ai_documents WHERE id = ANY/);
  assert.match(server, /normativa_operativa/);
  assert.match(server, /Decreto Legislativo n\.92 del 25 Maggio 2017/);
  assert.match(server, /Decreto Legislativo n\.211 del 10 Dicembre 2024/);
  assert.match(server, /Non trasformare una domanda normativa in una spiegazione di un campo app/);
  assert.match(server, /Per domande contabili, fiscali o economiche del compro oro separa sempre/);
  assert.match(server, /accounting_knowledge_loaded/);
  assert.match(server, /accounting_topics/);
  assert.match(server, /professional_gold_knowledge_loaded/);
  assert.match(server, /professional_gold_topics/);
  assert.match(server, /bullion_private_knowledge_loaded/);
  assert.match(server, /bullion_private_topics/);
  assert.match(server, /Modalita OPO-lingotti/);
  assert.match(server, /D\.Lgs\. 122\/2026 vigente dal 23 luglio 2026/);
  ...
  assert.match(server, /È necessario il consenso esplicito per salvare una memoria Aurum/);
  assert.match(server, /encryptAurumMemoryValue/);
...
test("Centro Privacy OroActive espone policy, presa visione e riferimenti cliente", async () => {
  ...
  assert.match(server, /const privacyPolicyVersion = "v1\.1"/);
...
test("deploy Coolify e aggiornamento PWA espongono versione e cache sicura", async () => {
  const [workflow, server, app, index, styles, worker, dockerfile, metadataScript, docs] = await Promise.all([
    ...
    file("service-worker.js"),
    file("Dockerfile"),
    file("scripts/write-version-metadata.mjs"),
  ...
  assert.match(workflow, /OROACTIVE_HEALTH_URL:-https:\/\/app\.oroactive\.it/);
  assert.match(workflow, /url\.pathname = "\/api\/health"/);
  assert.match(workflow, /seq 1 60/);
  assert.match(workflow, /publishedCommit = payload\?\.commit \|\| payload\?\.version\?\.commit \|\| ""/);
  assert.match(workflow, /publishedAssetBuildId = payload\?\.assetBuildId \|\| payload\?\.version\?\.assetBuildId \|\| ""/);
  assert.match(workflow, /publishedCatalogCount = Number\(payload\?\.catalogCount \|\| payload\?\.version\?\.catalogCount \|\| 0\)/);
  assert.match(workflow, /publishedCatalogCount === expectedCatalogCount/);
  ...
  assert.match(workflow, /deploy-check=\$\{GITHUB_RUN_ID\}-\$\{attempt\}/);
  ...
  assert.match(dockerfile, /RUN node scripts\/write-version-metadata\.mjs/);
  assert.match(metadataScript, /fs\.writeFileSync\("version\.json"/);
  assert.match(metadataScript, /rev-list", "--count", "HEAD"/);
  assert.match(metadataScript, /OROACTIVE_CLIENT_BUILD_ID/);
  assert.match(metadataScript, /EXPECTED_GOLD_COIN_CATALOG_COUNT/);
  assert.match(metadataScript, /assetBuildId/);
  assert.match(metadataScript, /catalogCount/);
  ...
  assert.match(server, /started_at: runtimeStatus\.startedAt/);
  assert.match(server, /assetBuildId: version\.assetBuildId/);
  assert.match(server, /catalogCount: version\.catalogCount/);
  ...
  assert.match(worker, /NEVER_CACHE_PATHS[\s\S]*"\/version\.json"/);
  assert.doesNotMatch(worker, /STATIC_ASSETS|cache\.addAll/);

  assert.match(app, /async function checkForAppUpdate/);
  assert.match(app, /const APP_VERSION_CHECK_INTERVAL_MS = 900000/);
  ...
  assert.match(app, /serverCatalogStale/);
  assert.match(app, /Nuova versione OroActive disponibile/);
  ...
  assert.match(index, /window\.__OROACTIVE_BOOT_BUILD_ID__/);
  assert.match(index, /resetStalePwa/);
  ...
  assert.match(docs, /https:\/\/app\.oroactive\.it\/api\/health/);
  assert.match(docs, /Include Source Commit in Build/);
  assert.match(docs, /\/api\/health[\s\S]*stesso commit appena inviato/);
