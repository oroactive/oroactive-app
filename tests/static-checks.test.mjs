...
test("quotazioni utenti copia cliente e refresh app aggiornati", async () => {
  const [index, app, server, schema, goldPredictionMigration, metalBuybackMigration, competitorMigration, competitorAiMigration, oroExpressMigration, oroDOroMigration, amicoOroMigration, prontoGoldMigration, bancoPreziosiMigration, bordinMigration, bordinGoldStandardMigration, oroInEuroMigration, competitorExtractionRulesMigration, bullionVaultProvider, aiCompetitorExtractor, oroExpressExtractor, oroDOroExtractor, amicoOroExtractor, prontoGoldExtractor, bancoPreziosiExtractor, bordinExtractor, goldStandardExtractor, oroInEuroExtractor, competitorExtractionTrainer] = await Promise.all([
    file("index.html"),
    file("app.js"),
    file("server.js"),
    file("schema.sql"),
  ...
  assert.match(app, /function competitorQuoteMatrixValue/);
  {
    ...
    assert.match(body, /<th>Caratura<\/th>/);
    assert.match(body, /<th>Purezza<\/th>/);
    ...
    assert.doesNotMatch(body, /Teorico oggi|Previsione 24h|Previsione 7g|Max pagabile oggi|Prezzo OroActive consigliato|Media competitor|Miglior competitor|Prezzo migliore di mercato|Scostamento vs|Stato competitività|Margine stimato|Scenario|Trend|Aurum|data-explain-price-row/);
  }
...
test("mascotte Aurum interattiva usa gufo dorato, flag e AI esistente", async () => {
  ...
  assert.match(index, /id="aurumPriceToolbar"/);
  assert.match(index, /data-aurum-price-followup="formula"/);
...
test("workflow autorizzazioni blocca pratiche rischiose e traccia Audit Trail", async () => {
  ...
  assert.match(worker, /competitor-matrix-1/);
});
...
test("qualita generale protegge click doppi messaggi tecnici e caricamenti sezione", async () => {
  ...
  assert.match(index, /app\.js\?v=20260607-competitor-matrix-1/);
  assert.match(index, /styles\.css\?v=20260607-competitor-matrix-1/);
  assert.match(worker, /competitor-matrix-1/);
