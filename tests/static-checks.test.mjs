...
test("quotazioni utenti copia cliente e refresh app aggiornati", async () => {
  ...
  assert.match(index, /Analisi Predittiva Metalli e Prezzi di Acquisto/);
  ...
  assert.match(app, /data-explain-price-row/);
  ...
  assert.match(app, /Prezzo migliore di mercato/);
  ...
  assert.match(app, /apiRequest\(`\/quotazioni\/competitors\/quotes\?days=30&currency=/);
...
test("workflow autorizzazioni blocca pratiche rischiose e traccia Audit Trail", async () => {
  ...
  assert.match(worker, /pronto-gold-1/);
});
...
test("qualita generale protegge click doppi messaggi tecnici e caricamenti sezione", async () => {
  ...
  assert.match(index, /app\.js\?v=20260606-pronto-gold-1/);
  assert.match(index, /styles\.css\?v=20260606-pronto-gold-1/);
  assert.match(worker, /pronto-gold-1/);
