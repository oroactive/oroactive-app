...
test("PWA non cachea API e dati sensibili", async () => {
  ...
  assert.match(sw, /\/pdf\//);
  assert.match(sw, /const BUILD_ID = "20260706-auth-main-menu-recovery-1"/);
  ...
  assert.match(index, /app\.js\?v=20260706-auth-main-menu-recovery-1/);
  assert.match(index, /styles\.css\?v=20260706-auth-main-menu-recovery-1/);
...
test("splash screen iniziale premium animata e senza ghost screen", async () => {
  ...
  assert.match(worker, /20260706-auth-main-menu-recovery-1/);
});
...
test("qualita generale protegge click doppi messaggi tecnici e caricamenti sezione", async () => {
  ...
  assert.match(loginBlock, /authAccepted = true;\s*await startAuthenticatedApp\(\);\s*schedulePostLoginMenuGuard\("login"\);\s*loginForm\.reset\(\);/);
