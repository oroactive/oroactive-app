...
test("Elenco Monete è una sottosezione Formazione con riconoscimento foto backend", async () => {
  ...
  assert.match(app, /Arca di Noe Armenia 2025 1 oz/);
  ...
  assert.match(app, /"sterlina-oro-sovrana"[\s\S]*"american-buffalo-1-oz"[\s\S]*"arca-noe-armenia-2025-1-oz"/);
...
test("workflow autorizzazioni blocca pratiche rischiose e traccia Audit Trail", async () => {
  ...
  assert.match(worker, /expanded-coin-front-back-1/);
});
...
test("qualita generale protegge click doppi messaggi tecnici e caricamenti sezione", async () => {
  ...
  assert.match(index, /app\.js\?v=20260609-expanded-coin-front-back-1/);
  assert.match(index, /styles\.css\?v=20260609-expanded-coin-front-back-1/);
  assert.match(worker, /expanded-coin-front-back-1/);
