...
test("PWA non cachea API e dati sensibili", async () => {
  ...
  assert.match(sw, /\/pdf\//);
  assert.match(sw, /const BUILD_ID = "20260729-gem-viewer-fix-2"/);
  ...
  assert.match(index, /meta name="oroactive-build-id" content="20260729-gem-viewer-fix-2"/);
  ...
  assert.match(index, /app\.js\?v=20260729-gem-viewer-fix-2/);
  assert.match(index, /styles\.css\?v=20260729-gem-viewer-fix-2/);
  assert.match(version, /"ok": true/);
  assert.match(version, /"assetBuildId": "20260729-gem-viewer-fix-2"/);
...
test("splash screen iniziale premium animata e senza ghost screen", async () => {
  ...
  assert.match(worker, /20260729-gem-viewer-fix-2/);
});
