...
test("PWA non cachea API e dati sensibili", async () => {
  ...
  assert.match(sw, /\/pdf\//);
  assert.match(sw, /const BUILD_ID = "20260702-austria-philharmonic-100-euro-1"/);
  ...
  assert.match(index, /app\.js\?v=20260702-austria-philharmonic-100-euro-1/);
  assert.match(index, /styles\.css\?v=20260702-austria-philharmonic-100-euro-1/);
...
test("splash screen iniziale premium animata e senza ghost screen", async () => {
  ...
  assert.match(worker, /20260702-austria-philharmonic-100-euro-1/);
});
...
test("menu principale usa macroaree centralizzate e permessi ruolo", async () => {
  ...
  assert.match(app, /if \(openMainMenu && !keepSplash\) showMainMenuFromSplash\(\);[\s\S]*void hydrateAuthenticatedAppInBackground\(\)\.catch/);
